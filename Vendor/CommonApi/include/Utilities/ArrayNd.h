...
namespace Utilities
{
    ...
    private:
        ...
        static inline constexpr std::array<std::size_t, dimensionAmount> multipliers = calculateMultipliers(dimensions);
    ...
    public:
        ...
        Type& operator()(Indices... indices) {
            return m_data[calculateIndex(std::forward<Indices>(indices)...)];
        };
        ...
        Type& at(Indices... indices) {
            if (!checkBounds(std::forward<Indices>(indices)...)) {
