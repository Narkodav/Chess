...
namespace Chess
{
        ...
        if (from.y == WHITE_EN_PASSANT_RANK)
        {
            ...
            if (checkBoundLeft(adjPos))
            {
                ...
                if (pieceLeft.piece == Piece::Type::BLACK_PAWN &&
                    pieceLeft.lastMoved == board.getTotalMoveCount() &&
                    pieceLeft.moveCounter == 1 &&
                    pieceLeftFront.isEmpty())
                {
                    Move move;
                    move.addedPieces.push_back(std::make_pair(adjPos + PAWN_DIRECTION_WHITE,
                        Piece(Piece::Type::WHITE_PAWN, currentPiece.moveCounter + 1, currentPiece.lastMoved + 1)));
            ...
            if (checkBoundRight(adjPos))
            {
                ...
                if (pieceRight.piece == Piece::Type::BLACK_PAWN &&
                    pieceRight.lastMoved == board.getTotalMoveCount() &&
        ...
        if (pieceNext.isEmpty())
        {
            if (nextPos.y == WHITE_PROMOTION_RANK)
            {
                ...
                move.addedPieces.push_back(std::make_pair(nextPos,
                    Piece(Piece::Type::WHITE_QUEEN, currentPiece.moveCounter + 1, currentPiece.lastMoved + 1)));
                move.removedPieces.push_back(std::make_pair(from, currentPiece));
                move.from = from;
                move.to = nextPos;
                moves.push_back(move);
            ...
            else
            {
                ...
                if (currentPiece.moveCounter == 0 && board.at(nextPos + PAWN_DIRECTION_WHITE).isEmpty())
                {
                    ...
                    move.from = from;
                    move.to = nextPos + PAWN_DIRECTION_WHITE;
    ...
    std::vector<Move> Calculator::getBlackPawnMoves(const Board& board, const glm::ivec2& from)
    {
            ...
            if (checkBoundLeft(adjPos))
            {
                    ...
                    pieceLeftFront.isEmpty())
                {
                    ...
                    move.addedPieces.push_back(std::make_pair(adjPos + PAWN_DIRECTION_BLACK,
                        Piece(Piece::Type::BLACK_PAWN, currentPiece.moveCounter + 1, currentPiece.lastMoved + 1)));
                    move.removedPieces.push_back(std::make_pair(from, currentPiece));
                    move.removedPieces.push_back(std::make_pair(adjPos, pieceLeft));
                    move.from = from;
        ...
        if (pieceNext.isEmpty())
        {
            if (nextPos.y == BLACK_PROMOTION_RANK)
            {
                Move move;
                move.addedPieces.push_back(std::make_pair(nextPos,
                    Piece(Piece::Type::BLACK_QUEEN, currentPiece.moveCounter + 1, currentPiece.lastMoved + 1)));
                move.removedPieces.push_back(std::make_pair(from, currentPiece));
                move.from = from;
                move.to = nextPos;
                moves.push_back(move);
            ...
            else
            {
                ...
                if (currentPiece.moveCounter == 0 && board.at(nextPos + PAWN_DIRECTION_BLACK).isEmpty())
                {
                    ...
                    moveNext.removedPieces.push_back(std::make_pair(from, currentPiece));
                    move.from = from;
                    move.to = nextPos + PAWN_DIRECTION_BLACK;
                    moves.push_back(moveNext);
                }
            }
        }
        ...
        if (from.y + 2 < 8)
        {
            ...
            if (nextPos.x < 8);
            {
                ...
                if (nextPiece.isEmpty())
                {
                    ...
                    move.removedPieces.push_back(std::make_pair(from, currentPiece));
                    move.addedPieces.push_back(std::make_pair(nextPos,
                        Piece(Piece::Type::WHITE_KNIGHT, currentPiece.moveCounter + 1, currentPiece.lastMoved + 1)));
                    moves.push_back(move);
                }
        ...
        if (from.y + 2 < 8)
        {
            ...
            if (nextPos.x < 8);
            {
                ...
                if (nextPiece.isEmpty())
                {
                    ...
                    move.removedPieces.push_back(std::make_pair(from, currentPiece));
                    move.addedPieces.push_back(std::make_pair(nextPos,
                        Piece(Piece::Type::BLACK_KNIGHT, currentPiece.moveCounter + 1, currentPiece.lastMoved + 1)));
                    moves.push_back(move);
                }
    ...
    std::vector<Move> Calculator::getWhiteBishopMoves(const Board& board, const glm::ivec2& from)
    {
        ...
        checkDirection(leftBottomLimit, board,
            from, moves, [](const Piece& piece) { return piece.isWhite(); },
            [](int i, const glm::ivec2& from) { return glm::ivec2(from.x - i, from.y - i); });

        checkDirection(leftTopLimit, board,
            from, moves, [](const Piece& piece) { return piece.isWhite(); },
            [](int i, const glm::ivec2& from) { return glm::ivec2(from.x - i, from.y + i); });

        checkDirection(rightBottomLimit, board,
            from, moves, [](const Piece& piece) { return piece.isWhite(); },
            [](int i, const glm::ivec2& from) { return glm::ivec2(from.x + i, from.y - i); });

        checkDirection(rightTopLimit, board,
            from, moves, [](const Piece& piece) { return piece.isWhite(); },
            [](int i, const glm::ivec2& from) { return glm::ivec2(from.x + i, from.y + i); });
    }
    ...
    std::vector<Move> Calculator::getWhiteRookMoves(const Board& board, const glm::ivec2& from)
    {
        ...
        checkDirection(upLimit, board, from, moves,
            [](const Piece& piece) { return piece.isWhite(); },
            [](int i, const glm::ivec2& from) { return glm::ivec2(from.x, from.y + i); });

        checkDirection(downLimit, board, from, moves,
            [](const Piece& piece) { return piece.isWhite(); },
            [](int i, const glm::ivec2& from) { return glm::ivec2(from.x, from.y - i); });

        checkDirection(leftLimit, board, from, moves,
            [](const Piece& piece) { return piece.isWhite(); },
            [](int i, const glm::ivec2& from) { return glm::ivec2(from.x - i, from.y); });

        checkDirection(rightLimit, board, from, moves,
            [](const Piece& piece) { return piece.isWhite(); },
            [](int i, const glm::ivec2& from) { return glm::ivec2(from.x + i, from.y); });
    }
    ...
    std::vector<Move> Calculator::getWhiteQueenMoves(const Board& board, const glm::ivec2& from)
    {
        ...
        diagonalMoves.insert(perpendicularMoves.begin(), perpendicularMoves.end(), diagonalMoves.end());
    ...
    std::vector<Move> Calculator::getWhiteKingMoves(const Board& board, const glm::ivec2& from)
    {
        ...
        for (int x = bottomX; x < topX; ++x)
        {
            for (int y = bottomY; y < topY; ++y)
            {
                ...
                if (movedTo.isWhite() || x == from.x && y == from.y)
                    continue;

                int leftBottomLimit = std::min(x, y);
                int leftTopLimit = std::min(x, 7 - y);
                int rightBottomLimit = std::min(7 - x, y);
                int rightTopLimit = std::min(7 - x, 7 - y);

                if (checkDirectionForEnemy(board, buf, leftBottomLimit,
                    [](const Piece& p) { return p.isWhite(); },
                ...
                if (checkDirectionForEnemy(board, buf, leftTopLimit,
                    [](const Piece& p) { return p.isWhite(); },
                    [](int i, const glm::ivec2& pos) { return glm::ivec2(pos.x - i, pos.y + i); },
                    [](const Piece& p, int i) {
                        return p.piece == Piece::Type::BLACK_BISHOP || p.piece == Piece::Type::BLACK_QUEEN
                ...
                if (checkDirectionForEnemy(board, buf, rightBottomLimit,
                    [](const Piece& p) { return p.isWhite(); },
                    [](int i, const glm::ivec2& pos) { return glm::ivec2(pos.x + i, pos.y - i); },
                    [](const Piece& p, int i) {
                        return p.piece == Piece::Type::BLACK_BISHOP || p.piece == Piece::Type::BLACK_QUEEN
                ...
                if (checkDirectionForEnemy(board, buf, rightTopLimit,
                    [](const Piece& p) { return p.isWhite(); },
                    [](int i, const glm::ivec2& pos) { return glm::ivec2(pos.x + i, pos.y + i); },
                    [](const Piece& p, int i) {
                        return p.piece == Piece::Type::BLACK_BISHOP || p.piece == Piece::Type::BLACK_QUEEN
                ...
                if (checkDirectionForEnemy(board, buf, upLimit,
                    [](const Piece& p) { return p.isWhite(); },
                ...
                if (checkDirectionForEnemy(board, buf, downLimit,
                    [](const Piece& p) { return p.isWhite(); },
                    [](int i, const glm::ivec2& pos) { return glm::ivec2(pos.x, pos.y - i); },
                    [](const Piece& p, int i) {
                        return p.piece == Piece::Type::BLACK_ROOK || p.piece == Piece::Type::BLACK_QUEEN
                ...
                if (checkDirectionForEnemy(board, buf, leftLimit,
                    [](const Piece& p) { return p.isWhite(); },
                ...
                if (checkDirectionForEnemy(board, buf, rightLimit,
                    [](const Piece& p) { return p.isWhite(); },
                    [](int i, const glm::ivec2& pos) { return glm::ivec2(pos.x + i, pos.y); },
                    [](const Piece& p, int i) {
                        return p.piece == Piece::Type::BLACK_ROOK || p.piece == Piece::Type::BLACK_QUEEN
                ...
                if (y + 2 < 8)
                {
                    if (x + 1 < 8 && board.at(x + 1, y + 2).piece == Piece::Type::BLACK_KNIGHT)
                        continue;
                    if (x - 1 > -1 && board.at(x - 1, y + 2).piece == Piece::Type::BLACK_KNIGHT)
                        continue;
                }
                if (y - 2 > -1)
                {
                ...
                if (x + 2 < 8)
                {
                    if (y + 1 < 8 && board.at(x + 2, y + 1).piece == Piece::Type::BLACK_KNIGHT)
                        continue;
                    if (y - 1 > -1 && board.at(x + 2, y - 1).piece == Piece::Type::BLACK_KNIGHT)
                        continue;
                }
                if (x - 2 > -1)
                {
    ...
    std::vector<Move> Calculator::getBlackKingMoves(const Board& board, const glm::ivec2& from)
    {
        ...
        for (int x = bottomX; x < topX; ++x)
        {
            for (int y = bottomY; y < topY; ++y)
            {
                ...
                if (y + 2 < 8)
                {
                    if (x + 1 < 8 && board.at(x + 1, y + 2).piece == Piece::Type::WHITE_KNIGHT)
                        continue;
                    if (x - 1 > -1 && board.at(x - 1, y + 2).piece == Piece::Type::WHITE_KNIGHT)
                        continue;
                }
                if (y - 2 > -1)
                {
                ...
                if (x + 2 < 8)
                {
                    if (y + 1 < 8 && board.at(x + 2, y + 1).piece == Piece::Type::WHITE_KNIGHT)
                        continue;
                    if (y - 1 > -1 && board.at(x + 2, y - 1).piece == Piece::Type::WHITE_KNIGHT)
                        continue;
                }
                if (x - 2 > -1)
                {
